...
## Decision 5: Layered Application Boundary (Controller -> Service -> Domain -> Infrastructure)

### Problem

Examiner concern: orchestration appears concentrated in one object and mixes core logic with persistence/analytics/provider concerns.

### Decision

Adopt and document explicit layering:

- **Presentation**: Flask route handlers as controllers (`routes/chat.py`, `routes/session.py`, `routes/voice.py`)
- **Application**: `SalesChatbot` as chat service facade coordinating one turn
- **Domain**: `SalesFlowEngine` and `analysis.py` decision logic
- **Infrastructure**: `SessionPersistence`, `SessionAnalytics`, `PerformanceTracker`, provider adapters

### Rationale

This separation matches current runtime behavior while making responsibilities reviewable:

- controllers validate transport concerns (HTTP, input shape, session headers)
- chat service coordinates the turn lifecycle
- FSM owns stage truth and transition decisions
- infrastructure handles side effects (I/O, tracking, external APIs)

This is reflected explicitly in `full_arch_diagram.puml`.

---

## Decision 6: Formal FSM Contract (States, Transition Rules, Guards)

### Problem

Calling something an FSM is weak unless states and transitions are explicitly defined and testable.

### Decision

Model FSM contract with:

- explicit `Stage` enum (`INTENT`, `LOGICAL`, `EMOTIONAL`, `PITCH`, `OBJECTION`, `OUTCOME`)
- explicit `Strategy` enum (`INTENT`, `CONSULTATIVE`, `TRANSACTIONAL`)
- transition rules with `trigger`, `guard_fn`, `min_turns`, `max_turns`
- priority overrides evaluated before standard rules

### Rationale

The implementation already follows this in `flow.py` (`FLOWS`, `ADVANCEMENT_RULES`, `_check_priority_overrides`). The architecture diagrams now expose that contract instead of hiding it behind one state variable.

---

## Decision 7: Single Source of Truth for Seller Flow vs Prospect Simulation

### Problem

Potential ambiguity when both a seller FSM (`SalesFlowEngine`) and buyer simulation (`ProspectSession`) maintain conversation state.

### Decision

Define clear ownership boundary:

- **Seller mode**: `SalesFlowEngine` is the only authority for seller stage progression
- **Prospect mode**: `ProspectSession` manages simulated buyer readiness only
- no cross-ownership of stage truth between these two brains

### Rationale

This avoids conflicting decision engines and gives a clear examiner answer to "who decides what happens next" in each mode.

---

## Decision 8: Closed-Loop Decision Pipeline (Analysis -> FSM -> Prompt -> Provider -> FSM)

### Problem

Class diagrams alone can hide runtime behavior; examiners need an executable control-flow narrative.

### Decision

Publish a runtime sequence artifact (`Documentation/diagrams/03_runtime_sequence.mmd`) that shows a single chat turn end-to-end:

1. request enters controller
2. state analysis runs
3. FSM checks override and transition rules
4. prompt is assembled
5. provider router executes with fallback
6. response is parsed and validated
7. FSM writes turn and applies advancement
8. session is persisted and analytics are recorded

### Rationale

This transforms architecture from static structure to auditable behavior, which is usually the key gap in project defenses.

---

## Decision 9: Provider Depth and Resilience as First-Class Design

### Problem

Simple provider abstraction appears shallow without explicit retry/fallback/error contracts.

### Decision

Document provider integration as a pipeline with dedicated responsibilities:

- routing (`ProviderRouter`)
- fallback/retry policy (`FallbackStrategy`)
- response normalization (`ResponseParser` + `LLMResponse`)
- unified error contract (`ErrorEnvelope`)

### Rationale

This maps directly to existing behavior (`_try_fallback_providers`, canonical provider error codes) and makes reliability design explicit for assessment.

---

## Decision 10: Voice Path as an Integrated Flow, Not a Sidecar

### Problem

Voice can look bolted-on if represented only as I/O wrappers.

### Decision

Model voice interaction as a composed orchestration:

- STT (`VoiceProvider.transcribe`) -> same chat decision loop -> TTS (`VoiceProvider.synthesize`)
- shared session and FSM state with text chat
- latency surfaced as a per-stage breakdown

### Rationale

Voice becomes a transport mode over the same decision core, not an alternative architecture branch.
