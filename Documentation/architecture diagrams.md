...
sequenceDiagram
    ...
    participant T as Training Coach
    ...
    C->>T: generate_training(user_msg, bot_reply)
    T->>L: provider.chat([coaching prompt])
    L-->>T: coaching JSON {stage_goal, what_bot_did, next_trigger, ...}
    T-->>C: training dict

    C-->>A: ChatResponse {content, latency_ms, stage}
    A-->>F: JSON {message, stage, strategy, metrics, training}
    F-->>U: Display message + stage badge + training panel
```
...
### 9r: Provider Hot-Swap Logic

> **Note:** Provider selection occurs at session initialisation via `create_provider()` in `chatbot.__init__()`. Runtime provider switching is achieved by setting the `LLM_PROVIDER` env var before starting the server. The flow below documents the hot-swap design; runtime swap capability can be added as a future enhancement.
