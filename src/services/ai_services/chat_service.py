...
from src.services.ai_services.persona_service import persona_service
from src.services.ai_services.model_service import model_service
from src.services.ai_services.langchain_conversation_service import get_langchain_service
from src.services.data_services.analytics_service import get_analytics_aggregator
from src.services.analysis_services.feedback_service import feedback_service
...
# Define PERFORMANCE_STATS as a dictionary for tracking performance metrics
PERFORMANCE_STATS = {
    "startup_time": 0,
    "total_requests": 0,
    "last_request_time": 0,
...
class ChatService:
  ...
  def _get_langchain_service(self):
      if self.langchain_service is None:
          try:
              self.langchain_service = get_langchain_service()
              self.use_langchain = True
              logger.info("🔗 LangChain service available (enabled on first use)")
          except Exception as e:
              self.langchain_service = None
              self.use_langchain = False
              logger.warning(f"🔗 LangChain service unavailable: {e}")
      return self.langchain_service
  ...
  def end_session(self, user_id: str, session_id: str, persona_name: str = None) -> Dict:
      ...
      session = self.active_sessions[session_key]
      end_time = time.time()
      duration = (end_time - session["start_time"]) / 60
  ...
  def get_conversation_stats(self) -> dict:
      ...
      if self.active_sessions:
        ...
        for session in self.active_sessions.values():
          ...
          duration = (current_time - session["start_time"]) / 60
...
def get_chat_service():
    """Factory function to get a singleton ChatService instance"""
