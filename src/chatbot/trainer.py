...
def answer_training_question(provider, flow_engine, question, style: str = "tactical"):
    ...
    recent = "\n".join(f"{m['role'].upper()}: {m['content']}" for m in flow_engine.conversation_history[-8:])
    
    # Select methodology based on flow type
    ...
    system_prompt = (
        ...
        f"Advance when: {rubric.get('advance_when', '')} | Concepts: {", ".join(rubric.get('key_concepts', []))}\n\n"
        f"Recent: {recent}\n\n{style_guide}"
    )
...
def score_session(session_id: str) -> dict:
    ...
    stages_reached, score_breakdown = set(), {k: 0 for k in ["stage_progression", "signal_detection", "objection_handling", "questioning_depth", "conversation_length"]}
