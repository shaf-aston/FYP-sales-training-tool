      ...
      function createMessageElement(text, sender, msgIdx, metrics = null) {
        ...
        // Preserve newlines as <br>; bot messages get markdown rendered
        const safeText = text || "";
      ...
      function updateTrainingPanel(training) {
        if (!training) return;
        document.getElementById("tStageGoal").textContent =
          training.stage_goal || "—";
        document.getElementById("tWhatBotDid").textContent =
          training.what_bot_did || "—";
        document.getElementById("tWhereHeading").textContent =
          training.where_heading || "—";
        document.getElementById("tNextTrigger").textContent =
      ...
      function displayQuizFeedback(data) {
        ...
        if (data.strengths && data.strengths.length) {
          ...
          data.strengths.forEach((s) => (html += `<li>${parseMarkdown(String(s))}</li>`));
          html += "</ul></div>";
        }
        if (data.improvements && data.improvements.length) {
          html += '<div class="quiz-details"><strong>Improvements:</strong><ul>';
          data.improvements.forEach((s) => (html += `<li>${parseMarkdown(String(s))}</li>`));
          html += "</ul></div>";
        }
        if (data.key_concepts_got && data.key_concepts_got.length) {
          html += '<div class="quiz-details"><strong>Concepts you got:</strong><ul>';
          data.key_concepts_got.forEach((s) => (html += `<li>${parseMarkdown(String(s))}</li>`));
          html += "</ul></div>";
        }
        if (data.key_concepts_missed && data.key_concepts_missed.length) {
          html += '<div class="quiz-details"><strong>Concepts to review:</strong><ul>';
          data.key_concepts_missed.forEach((s) => (html += `<li>${parseMarkdown(String(s))}</li>`));
          html += "</ul></div>";
        }
      ...
      async function loadDevState() {
        ...
        try {
          ...
          document.getElementById("dp-s-stageTurns").textContent = s.stage_turn_count ?? "—";
          document.getElementById("dp-s-totalTurns").textContent = s.total_turns ?? "—";
      ...
      async function devJumpStage(stage) {
        ...
        try {
          ...
          const d = await r.json();
          if (!d.success) { dpToast("Error: " + (d.error || "failed")); return; }
          updateStage(d.stage);
          updateStrategy(d.strategy);
          await loadDevState();
          dpToast("Jumped → " + stage.toUpperCase());
        } catch(e) { dpToast("Error: " + e); }
      }

      async function loadDevConfig() {
        if (_devConfig) { populateDevConfig(_devConfig); return; }
        try {
      ...
      async function devFetchPrompt() {
        const sid = getSessionId();
        if (!sid) { dpToast("No active session"); return; }
        ...
        try {
          const r = await fetch("/api/debug/prompt", { headers: { "X-Session-ID": sid } });
          const d = await r.json();
          if (d.error) { document.getElementById("dp-prompt-pre").textContent = "Error: " + d.error; return; }
        ...
        } catch(e) { document.getElementById("dp-prompt-pre").textContent = "Error: " + e; }
      }

      function devCopyPrompt() {
        if (!_devPromptText) return;
        navigator.clipboard.writeText(_devPromptText)
      ...
      async function devAnalyseMessage() {
        ...
        const message = input.value.trim();
        if (!message) return;
        if (!sid) { result.textContent = "No active session"; return; }
      ...
      function renderAnalysisResult(d) {
        ...
        const stateColor = v => v === "high" ? "dp-hit" : v === "low" ? "dp-miss" : "";
        ...
        // Signals
        const sigLabels = {
          high_intent: "High Intent", low_intent: "Low Intent",
          commitment: "Commitment", objection: "Objection",
          walking: "Walking Away", impatience: "Impatience",
          demands_directness: "Demands Directness", direct_info_requests: "Direct Info Req",
          user_consultative_signals: "→ Consultative", user_transactional_signals: "→ Transactional",
        };
        const sigItems = Object.entries(sigLabels).map(([k, label]) => {
        ...
        const waLabel = wa === null ? "N/A (terminal stage)" : wa ? "YES — would advance" : "NO — stays in stage";
        const advHtml = `
          <div class="dp-analyse-group">
      ...
      async function devApplyReinit() {
        ...
        const forceStrategy = document.getElementById("dp-strategy-sel").value || null;
        ...
        try {
          ...
          if (data.error) { dpToast("Init failed: " + data.error); return; }
      ...
      try {
        ...
        if (saved.showHints !== undefined) _prospectSettings.showHints = saved.showHints;
        if (saved.evalDisplay) _prospectSettings.evalDisplay = saved.evalDisplay;
      } catch {}

      function saveProspectSettings() {
        _prospectSettings.evalDisplay = document.getElementById("prospectEvalDisplay").value;
      ...
      function selectProspectDifficulty(diff, btn) {
        ...
        document.querySelectorAll(".prospect-diff-btn").forEach(b => b.classList.remove("selected"));
      ...
      async function startProspectMode() {
        ...
        try {
          ...
          document.getElementById("stageBadge").textContent = _prospectDifficulty.toUpperCase();
          ...
          document.getElementById("prospectBackground").textContent = data.persona.background;
          updateProspectDiffBadge(data.difficulty);
          updateProspectPanel(data.state);

          // Apply settings UI
          document.getElementById("prospectHintsToggle").classList.toggle("on", _prospectSettings.showHints);
          document.getElementById("prospectCoachingSection").style.display =
            _prospectSettings.showHints ? "" : "none";
          document.getElementById("prospectEvalDisplay").value = _prospectSettings.evalDisplay;
      ...
      function endProspectMode() {
        if (_prospectMode && _prospectSessionId) {
          fetch("/api/prospect/reset", {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-Session-ID": _prospectSessionId },
          }).catch(() => {});
        }
      ...
      function updateProspectPanel(state) {
        ...
        document.getElementById("prospectTurnCount").textContent = state.turn_count || 0;
      }
        ...
        fetch("/api/prospect/chat", {
          ...
          .then(data => {
            ...
            if (data.coaching && _prospectSettings.showHints) {
              document.getElementById("prospectCoachingHint").textContent = data.coaching.hint;
              document.getElementById("prospectCoachingSection").style.display = "";
            }
      ...
      async function requestProspectEvaluation() {
        ...
        loadingMsg.innerHTML = '<div class="message-bubble" style="color:#06b6d4">Generating evaluation...</div>';
        ...
        try {
          ...
          if (!data.success) {
            addProspectMessage("Evaluation failed: " + (data.error || "Unknown error"), "bot");
            return;
          }
      ...
      function renderEvalModal(html) {
        ...
        overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:300;display:flex;justify-content:center;align-items:center;overflow-y:auto;padding:20px";
        const card = document.createElement("div");
        card.className = "prospect-evaluation";
        card.style.cssText = "max-width:500px;width:100%;max-height:90vh;overflow-y:auto";
        card.innerHTML = html;
