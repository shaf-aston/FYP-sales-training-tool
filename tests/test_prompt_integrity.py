...
def _required_prompt_keys(strategy) -> set[str]:
    stages = {stage.value if hasattr(stage, "value") else str(stage) for stage in FLOWS[strategy]["stages"]}
    if "intent" in stages:
...
class TestPromptCoverage:
    def test_consultative_stage_prompts_exist(self):
        ...
        missing = [key for key in sorted(required) if not get_prompt("consultative", key)]
    ...
    def test_transactional_stage_prompts_exist(self):
        ...
        missing = [key for key in sorted(required) if not get_prompt("transactional", key)]
    ...
    def test_intent_discovery_uses_consultative_prompt_set(self):
        ...
        strategy_for_prompt = _strategy_key(engine.strategy_for_prompts)
