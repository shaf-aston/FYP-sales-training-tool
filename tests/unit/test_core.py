...
- Ack tactics: test_ack_tactics.py
...
from chatbot.loader import get_product_settings, load_product_config, load_signals
from chatbot.providers.base import BaseLLMProvider, LLMResponse
from chatbot.providers.factory import _get_providers_registry, create_provider
...
class TestKeywordMatching:
    ...
    def test_case_insensitive_match(self):
        ...
        assert contains_nonnegated_keyword(
            "JUST SHOW ME something", SIGNALS["impatience"]
        )
    ...
    def test_no_match_on_unrelated_text(self):
        assert not contains_nonnegated_keyword(
            "the weather is nice today", SIGNALS["impatience"]
        )
...
class TestPreferenceExtraction:
    ...
    def test_budget_and_safety_detected(self):
        history = [
            {
                "role": "user",
                "content": "I need something affordable and safe for my kids",
            },
...
class TestChatbotInit:
    ...
    def test_transactional_product(self):
        ...
        from src.chatbot.flow import Stage

        assert bot.flow_engine.flow_config["stages"] == [
            Stage.INTENT,
            Stage.PITCH,
            Stage.OBJECTION,
            Stage.OUTCOME,
...
class TestProviderFactory:
    ...
    def test_registry_has_groq(self):
        assert "groq" in _get_providers_registry()
