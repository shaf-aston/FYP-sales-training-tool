...
def test_generate_stage_prompt_component_order():
    ...
    pre_state = SimpleNamespace(
        decisive=False, intent="low", guarded=False, question_fatigue=False
    )
    ...
    # Ordering assertions: base -> stage -> recent conversation -> turn context
    # Turn context is placed last to preserve recency and metadata at end
    ...
    assert prod_idx != -1 and stage_idx != -1 and recent_idx != -1 and turn_idx != -1
    assert prod_idx < stage_idx < recent_idx < turn_idx

    # Tactic/adaptation guidance (low-intent consultative) appears before stage prompt
    assert ("TACTIC OVERRIDE" in prompt) or ("ADAPTATION" in prompt)
    tactic_idx = (
        prompt.find("TACTIC OVERRIDE")
        if "TACTIC OVERRIDE" in prompt
        else prompt.find("ADAPTATION")
    )
    assert tactic_idx < stage_idx


def test_chat_builds_messages_and_duplicates_history():
    bot = SalesChatbot(
        provider_type="dummy", model=None, product_type=None, session_id="ordering_test"
    )
    ...
    assert any(
        m
        for m in msgs[1:-1]
        if m.get("role") == "user" and "Hello" in m.get("content", "")
    )
