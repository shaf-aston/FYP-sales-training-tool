...
class TestProspectConfig:
    ...
    def test_behaviour_params_valid(self):
        from chatbot.loader import load_prospect_config

        config = load_prospect_config()
        for diff, profile in config["difficulty_profiles"].items():
            b = profile["behaviour"]
            assert 0.0 <= b["initial_readiness"] <= 1.0, (
                f"{diff}: invalid initial_readiness"
            )
            assert b["readiness_gain_per_good_turn"] > 0, (
                f"{diff}: gain must be positive"
            )
            assert b["readiness_loss_per_bad_turn"] > 0, (
                f"{diff}: loss must be positive"
            )
    ...
    def test_personas_have_required_fields(self):
        ...
        for category, persona_list in personas.items():
            for persona in persona_list:
                ...
                assert not missing, (
                    f"Persona '{persona.get('name', '?')}' in '{category}' missing: {missing}"
                )
    ...
    def test_easy_readiness_higher_than_hard(self):
        ...
        profiles = config["difficulty_profiles"]
        assert (
            profiles["easy"]["behaviour"]["initial_readiness"]
            > profiles["hard"]["behaviour"]["initial_readiness"]
        )
    ...
    def test_hard_has_more_objections_than_easy(self):
        ...
        profiles = config["difficulty_profiles"]
        assert (
            profiles["hard"]["behaviour"]["max_objections"]
            > profiles["easy"]["behaviour"]["max_objections"]
        )
    ...
    def test_system_prompt_template_exists(self):
        ...
        assert "{behaviour_rules}" in template
...
class TestProspectState:
    """Test ProspectState data structure behaviour."""
    ...
    def test_initial_readiness_matches_difficulty(self):
        ...
        for diff, profile in config["difficulty_profiles"].items():
            state = ProspectState(
                readiness=profile["behaviour"]["initial_readiness"],
                difficulty=diff,
            )
            assert state.readiness == profile["behaviour"]["initial_readiness"]
...
class TestPersonaSelection:
    ...
    def test_product_specific_persona_exists(self):
        ...
        assert persona["name"] in ("Marcus", "Priya") or persona["name"] in (
            "Alex",
            "Sam",
            "Jordan",
        )
...
class TestPromptBuilding:
    ...
    def test_persona_name_in_prompt(self):
        ...
        session.state = __import__(
            "chatbot.prospect", fromlist=["ProspectState"]
        ).ProspectState(readiness=0.5, difficulty="easy", persona=session.persona)
        config = __import__(
            "chatbot.loader", fromlist=["load_prospect_config"]
        ).load_prospect_config()
        session.difficulty_profile = config["difficulty_profiles"]["easy"]
        session.behaviour_rules = config.get("behaviour_rules", {}).get("easy", "")
    ...
    def test_difficulty_rules_in_prompt(self):
        ...
        session.state = ProspectState(
            readiness=0.2, difficulty="hard", persona=session.persona
        )
        session.difficulty_profile = config["difficulty_profiles"]["hard"]
        session.behaviour_rules = config.get("behaviour_rules", {}).get("hard", "")
