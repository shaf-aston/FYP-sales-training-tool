...
class TestGenerateTraining:
    ...
    def test_valid_json_returns_all_fields(self):
        ...
        result = generate_training(
            provider, _engine("logical"), "I've been struggling", "I hear you."
        )
    ...
    def test_error_response_falls_back(self):
        ...
        result = generate_training(
            MockProviderError(), _engine("pitch"), "ok", "great!"
        )
        assert "what_happened" in result
        assert isinstance(result["watch_for"], list)
    ...
    def test_provider_exception_falls_back(self):
        ...
        result = generate_training(
            MockProviderRaises(), _engine("intent"), "hello", "hi"
        )
        assert "what_happened" in result
        assert isinstance(result["watch_for"], list)
    ...
    def test_watch_for_coerced_to_list_when_string(self):
        ...
        result = generate_training(
            MockProvider(responses=[bad_json]), _engine("intent"), "x", "y"
        )
        assert isinstance(result["watch_for"], list)
    ...
    def test_markdown_json_block_stripped_and_parsed(self):
        ...
        wrapped = f"```json\n{VALID_COACHING_JSON}\n```"
        result = generate_training(
            MockProvider(responses=[wrapped]), _engine("logical"), "msg", "reply"
        )
        assert result["what_happened"] == "Asked an open-ended probe about the problem."
    ...
    def test_generate_training_word_limits(self):
        """Word limits enforced: what_happened/next_move ≤15 words, watch_for items ≤8 words."""
        long_json = json.dumps(
            {
                "what_happened": "This is a very long sentence that contains far more than fifteen words and should be truncated",
                "next_move": "Another extremely long next move instruction that exceeds the word limit and must be truncated by the function",
                "watch_for": [
                    "This is way too long for a watch for tip it should be truncated",
                    "Another super long watch for item that definitely exceeds eight words max",
                ],
            }
        )
        provider = MockProvider(responses=[long_json])
        result = generate_training(provider, _engine("intent"), "test", "test")

        # Check word counts
        what_happened_words = len(result["what_happened"].split())
        next_move_words = len(result["next_move"].split())
        assert what_happened_words <= 15, (
            f"what_happened has {what_happened_words} words, should be ≤15"
        )
        assert next_move_words <= 15, (
            f"next_move has {next_move_words} words, should be ≤15"
        )

        for tip in result["watch_for"]:
            tip_words = len(tip.split())
            assert tip_words <= 8, (
                f"watch_for tip has {tip_words} words, should be ≤8: {tip}"
            )
...
class TestAnswerTrainingQuestion:
    ...
    def test_provider_exception_returns_graceful_message(self):
        ...
        result = answer_training_question(
            MockProviderRaises(), engine, "Why pitch now?"
        )
        assert "answer" in result
        assert len(result["answer"]) > 0
...
class TestDevPanelPrompt:
    ...
    def test_prompt_includes_product_context(self):
        ...
        prompt = eng.get_current_prompt(user_message="")
        assert (
            "electric" in prompt.lower()
            or "vehicle" in prompt.lower()
            or len(prompt) > 100
        )
...
class TestDevPanelSignalAnalysis:
    ...
    def test_signal_hits_has_all_required_keys(self):
        ...
        signal_hits["demands_directness"] = user_demands_directness(
            engine.conversation_history, message
        )
    ...
    def test_high_intent_message_detected(self):
        ...
        assert contains_nonnegated_keyword(
            "I'm ready to buy a car", SIGNALS.get("high_intent", [])
        )
    ...
    def test_low_intent_browsing_detected(self):
        ...
        assert contains_nonnegated_keyword(
            "just browsing really", SIGNALS["low_intent"]
        )
