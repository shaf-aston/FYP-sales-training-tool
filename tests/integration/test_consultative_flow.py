...
class TestConsultativeFlowIntegration:
    ...
    def test_full_consultative_flow_requiresSIGNALS(self, chatbot):
        ...
        chatbot.chat(
            "I'm struggling to make consistent profits and looking for help improving my trading"
        )
        assert chatbot.flow_engine.current_stage == "logical", (
            "Should advance to logical after clear intent"
        )

        # Phase 3: Resist doubt for 9 turns → should STAY in logical
        for i in range(9):
            chatbot.chat(
                f"My current strategy is perfect and working great, iteration {i + 1}"
            )
            assert chatbot.flow_engine.current_stage == "logical", (
                f"Should stay in logical without doubt, turn {i + 1}"
            )

        # Phase 4: Express doubt → should advance to emotional
        chatbot.chat(
            "Actually, I'm really struggling with consistency and losing money"
        )
        ...
        assert chatbot.flow_engine.current_stage == "emotional", (
            "Should advance to emotional after expressing doubt"
        )
        ...
        chatbot.chat(
            "I'm really worried about my family's future and I feel stressed about money"
        )
        ...
        assert chatbot.flow_engine.current_stage == "pitch", (
            "Should advance to pitch after expressing stakes"
        )
    ...
    def test_safety_valve_prevents_infinite_loops(self, chatbot):
        ...
        chatbot.chat(
            "I'm struggling with my trading strategy and looking for a solution"
        )
        assert chatbot.flow_engine.current_stage == "logical"
        ...
        assert chatbot.flow_engine.current_stage == "emotional", (
            "Safety valve should trigger after 10 turns"
        )
    ...
    def test_early_commitment_skips_stages(self, chatbot):
        ...
        assert chatbot.flow_engine.current_stage != "logical", (
            "Should advance past logical with strong commitment"
        )
    ...
    def test_consultative_entry_via_mentorship_keyword(self):
        ...
        assert bot.flow_engine.flow_type == "consultative", (
            "Mentorship keyword should trigger consultative strategy"
        )
    ...
    def test_no_premature_pricing_discussion(self, chatbot):
        ...
        # Get to logical stage
        chatbot.chat(
            "I'm struggling with trading consistency and looking for a solution"
        )
        assert chatbot.flow_engine.current_stage == "logical"
    ...
    def test_training_coach_respects_consultative_stage(self, chatbot):
        ...
        # Get to logical stage
        chatbot.chat(
            "I'm struggling to grow my trading income and looking for professional help"
        )
        assert chatbot.flow_engine.current_stage == "logical"
        ...
        if any(word in training_text for word in pricing_mentions):
            ...
            assert (
                "don't" in training_text
                or "avoid" in training_text
                or "not" in training_text
    ...
    def test_rewind_functionality_preserves_stage(self, chatbot):
        ...
        assert chatbot.flow_engine.current_stage == initial_stage, (
            "Rewind should restore stage"
        )
        assert chatbot.flow_engine.stage_turn_count == initial_turns, (
            "Rewind should restore turn count"
        )
...
class TestTransactionalFlowComparison:
    ...
    def test_transactional_advances_faster(self, transactional_bot):
        ...
        assert transactional_bot.flow_engine.current_stage == "pitch", (
            "Transactional should advance to pitch quickly"
        )
