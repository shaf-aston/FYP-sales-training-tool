...
from chatbot.analysis import analyse_state, has_user_stated_clear_goal, is_literal_question, user_demands_directness
...
class TestPriority1IntentLock:
    ...
    def test_goal_priming_empty_history(self):
        ...
        assert not has_user_stated_clear_goal([])
        assert not has_user_stated_clear_goal(None)
    ...
    def test_intent_lock_prevents_reversion(self):
        ...
        assert state["intent"] == "high", f"Goal priming failed: intent={state['intent']}, expected HIGH"
    ...
    def test_intent_lock_multiple_goal_keywords(self):
        ...
        for keyword in goal_keywords:
            ...
            assert has_user_stated_clear_goal(history), f"Failed to detect goal keyword: {keyword}"
...
class TestPriority2LiteralQuestions:
    ...
    def test_literal_question_with_question_word(self):
        ...
        for question in literal_questions:
            assert is_literal_question(question), f"Failed to detect literal question: {question}"
    ...
    def test_literal_question_casual_format(self):
        ...
        for question in casual_questions:
            assert is_literal_question(question), f"Failed to detect casual question: {question}"
    ...
    def test_literal_question_with_trailing_question_mark(self):
        ...
        for question in questions:
            assert is_literal_question(question), f"Failed to detect question mark ending: {question}"
    ...
    def test_rhetorical_questions_excluded(self):
        ...
        for question in rhetorical:
            assert not is_literal_question(question), f"Incorrectly detected rhetorical as literal: {question}"
    ...
    def test_non_questions(self):
        ...
        for text in non_questions:
            assert not is_literal_question(text), f"Incorrectly detected non-question: {text}"

    def test_turn_40_scenario(self):
        """Test Turn 40 from user conversation: 'whats changed?'"""
        user_message = "whats changed?"
        assert is_literal_question(user_message), "Failed to detect Turn 40 'whats changed?' as literal question"
...
class TestPriority3FrustrationOverride:
    ...
    def test_frustration_demand_indicators(self):
        ...
        for msg in demand_messages:
            assert user_demands_directness(history, msg), f"Failed to detect demand: {msg}"
    ...
    def test_frustration_emotion_indicators(self):
        ...
        for msg in frustration_messages:
            assert user_demands_directness(history, msg), f"Failed to detect frustration: {msg}"
    ...
    def test_frustration_repeated_clarification(self):
        ...
        assert user_demands_directness(history, user_message), "Failed to detect repeated frustration"
    ...
    def test_turn_18_20_scenario(self):
        ...
        history = []
        for msg in messages:
            assert user_demands_directness(history, msg), f"Failed to detect frustration in Turn 18-20 scenario: {msg}"
    ...
    def test_normal_responses_not_flagged(self):
        ...
        for msg in normal_messages:
            assert not user_demands_directness(history, msg), f"Incorrectly flagged as frustration: {msg}"
    ...
    def test_case_insensitivity(self):
        ...
        history = []
        for msg in messages:
            assert user_demands_directness(history, msg), f"Case sensitivity failed: {msg}"
...
class TestPriority4ContextAwareGuardedness:
    ...
    def test_agreement_pattern_not_guarded(self):
        ...
        2. User gives substantial answer: 'I want to lose about twenty pounds this year',  # 9 words, substantive
        3. User says: 'ok'
        ...
        assert not state["guarded"], f"Incorrectly flagged agreement as guarded: {state}"
    ...
    def test_turn_28_agreement_scenario(self):
        ...
        state = analyse_state(history, "ok")
        assert not state["guarded"], "Turn 28: Incorrectly flagged 'ok' as guarded when agreeing"
    ...
    def test_actual_guardedness_still_detected(self):
        ...
        assert isinstance(state["guarded"], bool), "Guardedness detection should return bool"
    ...
    def test_substantive_answer_threshold(self):
        ...
        analyse_state(history_short, "ok")
        analyse_state(history_medium, "ok")
        state_long = analyse_state(history_long, "ok")

        # Long answer should be recognized as substantive
        assert not state_long["guarded"], "Should recognize 12+ word answer as substantive (not guarded)"
...
class TestIntegrationScenarios:
    ...
    def test_full_turn_14_16_scenario(self):
        ...
        state = analyse_state(conversation, "idk")

        # PRIORITY 1: Intent should stay HIGH due to goal priming
        assert state["intent"] == "high", f"Turn 14-16: Goal priming failed, intent={state['intent']}, expected HIGH"
    ...
    def test_full_turn_18_20_scenario(self):
        ...
        assert is_frustrated, "Turn 18-20: Failed to detect frustration demand"
    ...
    def test_full_turn_28_guarded_fix(self):
        ...
        state = analyse_state(conversation, "ok")

        # PRIORITY 4: 'ok' after substantive answer = agreement, not guarded
        assert not state["guarded"], f"Turn 28: Failed to recognize agreement pattern. Guarded={state['guarded']}"
    ...
    def test_full_turn_40_literal_question(self):
        ...
        assert is_question, "Turn 40: Failed to detect 'whats changed?' as literal question"
