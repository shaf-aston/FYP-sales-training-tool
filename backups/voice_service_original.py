...
try:
    ...
    # Catch broad exceptions to prevent import-time failures from optional deps (e.g., spaCy/thinc/NumPy ABI issues)
...
class EnhancedVoiceService:
    ...
    def __init__(self):
        logger.info("Initializing EnhancedVoiceService...")
        self.coqui_tts = None
        ...
        if TORCH_AVAILABLE:
            ...
            logger.info(f"Torch available. Using device: {self.device}")
        else:
            self.device = "cpu"
        ...
        # Initialize available services (moved from unreachable code in transcribe_speech)
    ...
    async def synthesize_speech(self,
                               ...
                               voice_clone_reference: Optional[str] = None) -> Dict:
        logger.info(f"Synthesizing speech for text: '{text[:50]}...' with emotion: {emotion}, speaker_id: {speaker_id}")
        """Route-friendly wrapper for TTS that returns base64 audio and timings.

        Args:
            text: Input text to synthesize.
            emotion: Emotion string (e.g., 'neutral', 'friendly').
            speaker_id: Optional voice id/persona mapping.
            voice_clone_reference: Optional reference audio path (unused in local mode).

        Returns:
            Dict containing 'audio_data' (base64) and 'processing_time'.
        """
        start_time = time.time()

        # Map emotion string to enum safely
        emotion_value = (emotion or "neutral").lower().strip()
        try:
            emotion_enum = VoiceEmotion(emotion_value)
        except Exception:
            emotion_enum = VoiceEmotion.NEUTRAL

        # Choose a reasonable default speaker voice
        speaker_voice = speaker_id or "female_1"

        audio_bytes = await self.text_to_speech(text, emotion_enum, speaker_voice)
        ...
        # Simple quality heuristic (length-based)
        quality_score = min(1.0, max(0.1, len(audio_bytes) / (16000 * 2)))
    ...
    async def transcribe_speech(self,
        ...
        logger.info(f"Transcribing speech with detect_emotion: {detect_emotion}, language: {language}")
        """Route-friendly wrapper for STT that accepts base64 audio.

        Args:
            audio_data: Base64 string or raw bytes of audio.
            detect_emotion: Whether to include emotion indicators.
            language: Target language code.

        Returns:
            Dict containing transcription fields and 'processing_time'.
        """
        start_time = time.time()
        ...
        if isinstance(audio_data, str):
            ...
            try:
                ...
                # If not valid base64, treat as raw text path
                audio_bytes = audio_data.encode("utf-8", errors="ignore")
        else:
            audio_bytes = audio_data

        result = await self.speech_to_text(audio_bytes, language=language)

        processing_time = time.time() - start_time

        if not result:
            logger.error(f"Speech transcription failed.")
            return {
                "text": "",
                "confidence": 0.0,
                "language": language,
        ...
        logger.info(f"Successfully transcribed speech: '{result.get("text", "")[:50]}...' in {processing_time:.2f} seconds.")
        ...
        return {
            ...
            "processing_time": processing_time,
            "emotion": result.get("emotion_indicators", {}).get("dominant_emotion") if detect_emotion else None
        }

        if detect_emotion:
            # Map indicators to a coarse emotion if available
            indicators = result.get("emotion_indicators", {})
            persona_attrs = {}
            if hasattr(self, "_determine_contextual_emotion"):
                try:
                    detected_emotion = self._determine_contextual_emotion(indicators, persona_attrs)
                except Exception:
                    detected_emotion = "neutral"
            else:
                detected_emotion = "neutral"
            out["emotion"] = detected_emotion if isinstance(detected_emotion, str) else getattr(detected_emotion, "value", "neutral")
    ...
    def _determine_contextual_emotion(self, context: Dict, persona_attrs: Dict) -> VoiceEmotion:
        """Determine appropriate emotion based on context"""
